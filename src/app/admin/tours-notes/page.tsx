...
export default function ToursNotes() {
    const { content: initialData, isLoading, isError } = useToursNotes();
    ...
    useEffect(() => {
        if (submitTriggered && editorData) {
            const handleSubmit = async () => {
                try {
                    const response = await axios.post('/api/text-content/save-tours-notes', { data: editorData });
    ...
    return (
        <AdminLayout>
            ...
            {isLoading ? (
                ...
                <EditorComponent initialData={editorData} setEditorData={setEditorData} triggerSubmit={triggerSubmit} />
            )}
        </AdminLayout>
    );
}
